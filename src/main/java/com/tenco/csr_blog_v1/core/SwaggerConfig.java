...
public class SwaggerConfig {
    ...
    // 보안 스키마 이름
    private static final String JWT_SCHENE_NAME = "jwtAuth";
    ...
    public OpenAPI openAPI(){
        ...
        SecurityRequirement securityRequirement = new SecurityRequirement().addList(JWT_SCHENE_NAME);

        // 3 보안 스키마 정의(HTTP Bearer 방식, 포맷은 JWT)
        // 이 설정이 있어야 Swagger UI 측 상단에 인증 버튼이 생긴다
        Components components = new Components()
                .addSecuritySchemes(JWT_SCHENE_NAME,
                        new SecurityScheme()
