...
public class GlobalValidationHandler {
    ...
    public void badRequestAdvice(JoinPoint jp) {
        // JoinPoint는 가로채기 당한 실제 컨트롤러의 메서드에 메타데이터 (이름, 매개변수, 리턴타입 ...) 다 가지고 있다

        // 가로채기 당한 메서드로 들어온 실제 인자값(객체들)을 배열로 싹 긁어 모아 옴
        Object[] args = jp.getArgs();
        for (Object arg : args) {

            // 긁어 모은 인자 중에 스프링이 유효성 검사 결과를 담아둔 Errors 객체타입이 존재한다면
            if (arg instanceof Errors) {
                ...
                if (errors.hasErrors()) {
                    ...
                    // 발견된 에러리스트 중 첫번째 에러를 꺼내서 '필드명: 에러메세지" 형태로 예외 내용을
